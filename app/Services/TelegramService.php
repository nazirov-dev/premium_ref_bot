    ...
    private function sendAPIRequest(string $url, array $content, bool $post = true): array
    {
        try {
            ...
            if ($result->successful()) {
                ...
                // Log both the status code and the error details
                $errorDetails = $result->body();
                $this->log('Error status: ' . $result->status() . ";\nError details:" . $errorDetails . ";\nError url: " . $url . ";\nData: " . json_encode($content, 128));
                return json_decode($errorDetails, true);
            }
        } catch (\Exception $e) {
            $this->log($e);
            return [];
